...
csv_path = "ppi3d.csv"
weights = "/home/slab/ishiiayuka/M2/Decoder/t30_150M_decoder_NAR_500epoch.pt"
protein_feat_path = "/home/slab/ishiiayuka/M2/Decoder/t30_150M.pt"
output_path = "/home/slab/ishiiayuka/M2/Decoder/t30_150M_decoder_NAR_after_reinforce_ppi3d_1015.pt"
...
def mean_plddt_from_pdb(pdb_path: str, residue_weighted: bool = True) -> float:
    """
    RF2NA出力PDBのB-factor列(=pLDDT, 0..100)から平均pLDDTを返す。
    residue_weighted=True: 残基平均→全残基平均（推奨）
    """
    if not os.path.exists(pdb_path):
        return 0.0

    if not residue_weighted:
        s = 0.0; n = 0
    ...
    with open(pdb_path) as fh:
        for line in fh:
            ...
            chain = line[21]             # 列22
            resi  = line[22:26].strip()  # 列23–26
            icode = line[26].strip()     # 列27
            key = (chain, resi, icode)
            res_S[key] = res_S.get(key, 0.0) + b
            res_N[key] = res_N.get(key, 0) + 1
    if not res_S:
        return 0.0
    per_res = [res_S[k]/res_N[k] for k in res_S]
