...
from sklearn.metrics import roc_auc_score, average_precision_score, accuracy_score, precision_score, recall_score, matthews_corrcoef, confusion_matrix, f1_score
...
class ZHMolGraph():
    ...
    def get_benchmark_validation_ZHMolGraph_results(self, rna_embedding_length=640, protein_embedding_length=1024, dataset=None, 
        ...
        performance_df.to_csv(performance_df_file, index=False, sep=',') 
    ...
    def get_TheNovel_test_results(self, graphsage_path=None, test_dataframe=None, model_dataset=None,
        ...
        for run in range(5):
            ...
            best_vecnet_model = nn.DataParallel(best_vecnet_model)
    ...
    def get_test_graphsage_embeddings(self, train_set, test_set, rnas, proteins, dataset, rnas_length, proteins_length, embedding_type, graphsage_embedding=1, graphsage_model_path=None):
        ...
        '''
        train_interaction_df = pd.concat([train_set['RNA_aa_code'], train_set['target_aa_code']], axis=1)  
        test_interaction_df = pd.concat([test_set['RNA_aa_code'], test_set['target_aa_code']], axis=1)  

        if not os.path.exists(dataset + '_' + embedding_type + '_graphsage_dataset/'):
            os.makedirs(dataset + '_' + embedding_type + '_graphsage_dataset/')
        total_interaction_df = pd.concat([train_interaction_df, test_interaction_df], axis=0) 
    ...
    def get_unseen_graphsage_embeddings(self, config='./graphsage_src/experiments.conf',
        ...
        graphsage = nn.DataParallel(graphsage)
        graphsage.to(device)


        node_map, feat_data = graphsage.module.dataCenter.load_dataSet(model_dataset, embedding_type)
        ...
        dataCenter.load_dataSet(ds, embedding_type=embedding_type) 
    ...
    def get_ProtTrans_embeddings(self, prediction_interactions=None, embedding_dimension=1024, replace_dataframe=True,
        ...
        for idx, protein in tqdm(enumerate(protein_list)):
            ...
            emb_0 = embedding_repr.last_hidden_state[0, :-1]  
