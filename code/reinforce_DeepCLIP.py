...
def valid_mask_upto_eos(tokens, pad_id, eos_id):
    # 有効: <eos>まで（<eos>含む）かつ <pad>でない位置
    not_pad = (tokens != pad_id).float()
    eos_cum = (tokens == eos_id).float().cumsum(dim=1)  # [B,L]
    before = (eos_cum == 0).float()
    at_eos = (tokens == eos_id).float()
    valid = (before + at_eos).clamp(max=1.0)
    return not_pad * valid  # [B,L]

def logprob_from_logits_upto_eos(logits, tokens, pad_id, eos_id):
    logp_tok = logits.log_softmax(-1).gather(-1, tokens.unsqueeze(-1)).squeeze(-1)  # [B,L]
    mask = valid_mask_upto_eos(tokens, pad_id, eos_id)
    return (logp_tok * mask).sum(dim=-1), mask  # [B], [B,L]
...
#DeepCLIPのときは以下を使う
config.protein_feat_path = "/home/slab/ishiiayuka/M2/Decoder/weights/t30_150M_deepclip.pt"
if torch.cuda.is_available():
    torch.cuda.manual_seed_all(42)
    dataset_train = DeepCLIPProteinDataset(
    feat_pt_path=config.protein_feat_path,
    csv_path=config.csv_path,
    id_priority=("uniprot_id", "protein_name", "file_name")
)
...
state_dict = torch.load("/home/slab/ishiiayuka/M2/Decoder/t30_150M__decoder_500_after_reinforce.pt", map_location="cpu")
...
for step in tqdm(range(max_steps),desc="step",position=0):
    #DeepCLIPのときは以下 
    for batch_idx, (protein_feat, tgt_seq, protein_seq_list, targets_parallel, lengths_parallel) in enumerate(tqdm(train_loader, desc="Samples", position=1, leave=False)):
        ...
        # ========= 4) logp（勾配あり）=========
        logp_batch, mask_valid = logprob_from_logits_upto_eos(logits, tokens, pad_id=pad_id, eos_id=eos_id)
        ...
        entropy_pos = -(probs * logits.log_softmax(-1)).sum(dim=-1)  # [B,L]
        entropy = (entropy_pos * mask_valid).sum(dim=-1) / (mask_valid.sum(dim=-1) + 1e-8)  # [B]
        loss = -(advantage * logp_batch).mean() - ENTROPY_BETA * entropy.mean()
        ...
        if step % 5 == 0:  # 例: 5ステップごと
            torch.save(
                ...
                "/home/slab/ishiiayuka/M2/Decoder/t30_150M__decoder_500_after_reinforce_deepclip.pt"
            )