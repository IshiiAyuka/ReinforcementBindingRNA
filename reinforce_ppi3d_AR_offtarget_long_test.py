...
def main():
    ...
    for step in tqdm(range(max_steps), desc="Steps"):
        ...
        lengths = torch.tensor(
            [len(s) for s in rna_strs],
            dtype=torch.float32,
            device=device
        )  # [B]

        # L_target = 30 に対して、ズレ |L_i - L_target| を計算
        len_diff = (lengths - LENGTH_TARGET).abs()  # [B]
        r_len = 1.0 - (len_diff / LENGTH_DIFF_MAX)  # [B]

        # ========= 3.2) オフターゲット（クロスバッチ平均）=========
        ...
        # ========= 3.3) 有効報酬 =========
        # R_eff = オンターゲット − オフターゲット + 長さ報酬
        R_eff = R - OFFTARGET_LAMBDA * R_off + LENGTH_LAMBDA * r_len  # [B]
